...
def main():
    ...
    # Load tokenizer
    ################
    tokenizer = get_tokenizer(model_name_or_path=model_name_or_path, set_pad_token=True)  # Always set pad token
    ...
    # Apply chat template to convert input/output to text format
    ...
    logger.info(f"Evaluation dataset size after filtering: {len(eval_dataset)}")

    time_string = time.strftime("%Y_%m_%d_%H%M%S", time.localtime())
    save_path = f"{sft_config.output_dir}/model_{time_string}"
    os.makedirs(save_path, exist_ok=True)

    # Remove the hard-coded dataset size limits for flexibility
    ...
    data_collator = create_data_collator(tokenizer, max_length=2048)
    ...
    trainer = SFTTrainer(
        ...
        data_collator=data_collator if not sft_config.packing else None,  # Use custom collator only when not packing
    )
